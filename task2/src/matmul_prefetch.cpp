...
void prefetch_line(const float* p) {
...
// Fast horizontal sum of an __m256
float hsum256(__m256 v) {
    __m128 vlow = _mm_add_ps(_mm256_castps256_ps128(v), _mm256_extractf128_ps(v, 1));    
...
float dot_kernel_1x(const float* a, const float* b1,
...
inline void dot_kernel_4x1(const float* a0, const float* a1, const float* a2, const float* a3, const float* b1, int start_idx_k, int end_idx_k,
            ...
            for (int start_idx_j = 0; start_idx_j < N; start_idx_j += b) {
                ...
                for (; i + 3 < end_idx_i; i += 4) {
                    ...
                    for (int j = start_idx_j; j < end_idx_j; j++) {
                        ...
                        float s0, s1, s2, s3;
                        dot_kernel_4x1(a0, a1, a2, a3, b1, start_idx_k, end_idx_k, s0, s1, s2, s3);
